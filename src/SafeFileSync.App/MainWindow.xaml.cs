...
public partial class MainWindow : Window
{
    ...
    private async Task Run(bool copy, JobInfo? resumedJob = null, bool failedOnly = false)
    {
        ...
        SetPlacement(legacy);
        if (sourceRows.Count == 0) { Status.Text = "원본 폴더를 1개 이상 추가하세요."; Summary.Text = "전송·검증이 시작되지 않았습니다."; return; }
        if (sourceRows.Any(row => string.IsNullOrWhiteSpace(row.Path) || (!legacy && string.IsNullOrWhiteSpace(row.FolderName))))
        { Status.Text = "모든 원본 경로와 목적지 안의 폴더 이름을 입력하세요. 빈 행은 제거할 수 있습니다."; Summary.Text = "전송·검증이 시작되지 않았습니다."; return; }
    ...
    private void RefreshHistory() { try { History.ItemsSource = coordinator.History().Select(j => new HistoryRow(j, $"{j.StartedUtc} · {LocalStatus(j.Status)} · 원본 {j.Sources?.Count ?? 1}개{(j.Sources is null ? " (이전 배치)" : "")} · {j.Destination}")).ToArray(); if (History.Items.Count > 0) History.SelectedIndex = 0; } catch (Exception ex) { Status.Text = "작업 이력 읽기 실패: " + ex.Message; } }
