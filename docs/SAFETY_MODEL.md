...
## Invariant
The app never intentionally deletes, moves, renames, truncates, writes or creates anything inside the selected source. No source metadata setters exist. Source data opens request read access only. The desktop app stores reports and SQLite databases under SafeFileSync in the Windows system-drive root (normally C:\), after source/storage separation checks. This is an internal location with no desktop setting or preview. Infrastructure callers can still supply an explicit parent for isolated fixtures. Windows access-time behavior is outside the app's control; PASS refers to the observed manifest scope, not an OS snapshot.

실제 내부 기록 폴더는 보통 `C:\SafeFileSync`입니다. 시작·이력 조회에서는 폴더를 생성하지 않습니다. 작업 시작 시 모든 원본·목적지와 문자열·물리 경로 겹침을 확인한 뒤에만 기록 폴더를 만듭니다. 기본 위치의 신규 폴더는 상속을 차단한 DACL에 현재 사용자 SID·SYSTEM·Administrators의 FullControl을 지정하고 하위 파일·폴더에 상속합니다. 기존 기본 폴더는 소유자·허용 ACL을 검사한 뒤 이력·DB를 읽거나 재사용하며 기존 ACL을 수정하지 않습니다. 생성 권한이 없거나 기존 권한 검사를 통과하지 못하면 S04로 차단합니다. 화면은 짧은 코드와 정해진 안내만 표시하며 예외 메시지의 내부 경로를 표시하지 않습니다.

alpha.5 기본 위치 이력은 같은 내부 위치에서 자동으로 조회·재개합니다. 이전 AppData 또는 사용자 지정 위치 기록의 자동 이동·삭제나 AppData로의 자동 복귀는 없으며, 새 화면에서는 해당 위치를 선택하지 않습니다. 시스템 드라이브 전체가 원본·목적지이면 내부 기록 위치도 포함되므로 S03으로 차단하며, 사용자는 필요한 전송 폴더를 선택해야 합니다. UI 설정 제거를 위해 겹침 검사나 권한 검사를 완화하지 않습니다.
