...
현재 버전은 **0.1.0-alpha.5**입니다. 자동 검증과 현장 검증 범위는 [검증 기록](docs/VALIDATION.md)을 확인하세요.
...
기본 **기록 위치**는 Windows가 설치된 드라이브의 최상위 폴더이며, 보통 `C:\`입니다. 실제 기록은 `C:\SafeFileSync`에 저장하므로 사용자 폴더 전체를 원본으로 선택해도 AppData와 겹치지 않습니다. 화면에서 실제 저장 위치를 확인하거나 `기본 위치`로 되돌릴 수 있습니다. `C:\` 전체가 원본이면 다른 드라이브에서 모든 원본·목적지 밖의 쓰기 가능한 기존 폴더를 선택하세요.
...
작업 DB·Unicode Robocopy 로그·HTML 보고서는 화면의 `기록 위치` 아래 `SafeFileSync`에 저장합니다. 기본 실제 위치는 보통 `C:\SafeFileSync`입니다. 앱 시작이나 이력 조회만으로 폴더를 만들지 않으며, 작업 시작 후 모든 원본·목적지와 겹치지 않는지 검사한 다음 생성합니다.

기본 위치에 새로 만드는 기록 폴더는 현재 사용자·SYSTEM·Administrators만 허용하도록 권한을 지정합니다. 기존 기본 기록 폴더는 소유자와 허용 권한을 검사하고 ACL을 자동 변경하지 않습니다. 생성 권한이 없거나 기존 권한이 안전하지 않으면 `S04`를 표시합니다. 이때 모든 원본·목적지 밖의 다른 쓰기 가능한 폴더를 선택하세요. AppData로 자동 전환하지 않으며, 직접 선택한 다른 기록 기준 폴더는 기존 권한을 유지합니다.

이전 AppData 기록은 이동하거나 삭제하지 않습니다. 이전 이력을 보려면 `%LOCALAPPDATA%`에 해당하는 실제 폴더 경로를 `기록 위치`에 다시 선택하세요. 사용자 지정 위치는 앱 재실행 후 다시 선택해야 합니다. 이력은 최근 100개 작업, 화면은 차이 항목을 우선한 최대 1,000개 항목을 표시합니다. 전체 결과는 HTML 보고서에 포함됩니다.
